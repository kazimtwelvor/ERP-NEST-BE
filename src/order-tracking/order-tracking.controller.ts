...
export class OrderTrackingController {
  ...
  @Permissions(AccessPermissions.ReadOrder)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Get all order items with optional filtering, search, and visibility checks' })
  @ApiQuery({ name: 'storeName', required: false, description: 'Store name to filter by' })
  @ApiQuery({ name: 'status', required: false, description: 'Current status to filter by' })
  @ApiQuery({ name: 'departmentId', required: false, description: 'Department ID to filter by' })
  @ApiQuery({ name: 'search', required: false, description: 'Search term for product name, SKU, or external order ID' })
  @ApiQuery({ name: 'productName', required: false, description: 'Product name to search for' })
  @ApiQuery({ name: 'sku', required: false, description: 'SKU to search for' })
  @ApiQuery({ name: 'color', required: false, description: 'Color to filter by' })
  @ApiQuery({ name: 'size', required: false, description: 'Size to filter by' })
  @ApiQuery({ name: 'externalOrderId', required: false, description: 'External order ID to search for' })
  @ApiQuery({ name: 'externalItemId', required: false, description: 'External item ID to search for' })
  @ApiQuery({ name: 'roleId', required: false, description: 'Role ID for visibility check' })
  @ApiQuery({ name: 'roleName', required: false, description: 'Role name for visibility check' })
  @ApiQuery({ name: 'roleIds', required: false, description: 'Array of role IDs for visibility check', type: [String] })
  @ApiQuery({ name: 'roleNames', required: false, description: 'Array of role names for visibility check', type: [String] })
