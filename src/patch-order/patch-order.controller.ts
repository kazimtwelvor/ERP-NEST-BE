...
export class PatchOrderController {
	...
	@Post(':id/notes')
	@HttpCode(HttpStatus.CREATED)
	@ApiOperation({ summary: 'Add a note to patch order' })
	@ApiParam({ name: 'id', description: 'Patch order ID (UUID)' })
	...
	@Get(':id/notes')
	@HttpCode(HttpStatus.OK)
	@ApiOperation({ summary: 'Get notes history for patch order' })
	@ApiParam({ name: 'id', description: 'Patch order ID (UUID)' })
	@ApiResponse({ status: 200, description: 'Notes fetched successfully' })
	@ApiResponse({ status: 404, description: PATCH_ORDER_MESSAGES.NOT_FOUND })
